...
class IFrame {
  ...
  createTab(title, link, uniqueالاسم, autoOpen) {
    const tabId = `panel-${uniqueالاسم}-${Math.floor(Math.random() * 1000)}`
    const navId = `tab-${uniqueالاسم}-${Math.floor(Math.random() * 1000)}`
  ...
  switchTab(item) {
    ...
    if (this._config.autoItemمتوفر) {
      this._setItemمتوفر($(`${tabId} iframe`).attr('src'))
    }
  }
  ...
  static _jQueryInterface(operation, ...args) {
    ...
    if (typeof operation === 'string' && operation.match(/createTab|openTabSidebar|switchTab|removeمتوفرTab/)) {
      data[operation](...args)
    }
  }
}
